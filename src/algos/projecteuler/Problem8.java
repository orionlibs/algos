package algos.projecteuler;

//Largest product in a series = https://projecteuler.net/problem=8
public class Problem8
{
    public static void main(String[] args)
    {
        System.out.println(solve());
    }
    
    
    private static long solve()
    {
        long max = 0;
        String x = "";
        
        for(int i = 0; i < x.length() - 13; i++)
        {
            long temp = Long.parseLong(Character.toString(x.charAt(i)))
                            * Long.parseLong(Character.toString(x.charAt(i + 1)))
                            * Long.parseLong(Character.toString(x.charAt(i + 2)))
                            * Long.parseLong(Character.toString(x.charAt(i + 3)))
                            * Long.parseLong(Character.toString(x.charAt(i + 4)))
                            * Long.parseLong(Character.toString(x.charAt(i + 5)))
                            * Long.parseLong(Character.toString(x.charAt(i + 6)))
                            * Long.parseLong(Character.toString(x.charAt(i + 7)))
                            * Long.parseLong(Character.toString(x.charAt(i + 8)))
                            * Long.parseLong(Character.toString(x.charAt(i + 9)))
                            * Long.parseLong(Character.toString(x.charAt(i + 10)))
                            * Long.parseLong(Character.toString(x.charAt(i + 11)))
                            * Long.parseLong(Character.toString(x.charAt(i + 12)));
            
            max = (temp > max) ? temp : max;
        }
        
        return max;
    }
}